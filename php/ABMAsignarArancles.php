...
if($tipo=="Especificos" || $tipo=="Materiales Didacticos" || $tipo=="Libros" ){
	...
	$condiciontipo="and (ar.cod_arancel='0' or (lta.tipo='$tipo' and ar.cod_carreraFk='$cod_carrera'))"; 
		...
		IF(ar.cod_arancel=0,IFNULL((select sum(monto + descuento) from facturaspagadas fac inner join cursosalumno cur on cur.idcursosalumno=fac.idcursosalumnoFk where cur.idalumnoFk='$idalumnofk' and fac.cod_arancelFk='0' and fac.estado='Activo' and fac.estadofactura='Activo' limit 1),0),IFNULL((select sum(fac.monto + fac.descuento) from facturaspagadas fac where fac.cod_arancelFk=ar.cod_arancel and fac.idcursosalumnoFk='$idcursosalumnoFk' and fac.estado='Activo' and fac.estadofactura='Activo' ),0)) as totalPagado,
		IF(ar.cod_arancel=0,IFNULL((select sum(descuento) from facturaspagadas fac inner join cursosalumno cur on cur.idcursosalumno=fac.idcursosalumnoFk where cur.idalumnoFk='$idalumnofk' and fac.cod_arancelFk='0' and fac.estado='Activo' and fac.estadofactura='Activo' limit 1),0),IFNULL((select sum(fac.descuento) from facturaspagadas fac where fac.cod_arancelFk=ar.cod_arancel and fac.idcursosalumnoFk='$idcursosalumnoFk' and fac.estado='Activo' and fac.estadofactura='Activo' ),0)) as descuento,
		(select nombre from filial where cod_filial=cr.cod_filialOringFK) as nombrefilial
        from aranceles ar inner join carrera cr on cr.cod_carrera=ar.cod_carreraFK
		inner join listadecarreras  ltc on ltc.Cod_listadecarreras=cr.Cod_listadecarrerasFK
		inner join listadearanceles lta on lta.cod_listadearanceles=ar.cod_listadearancelesFk
