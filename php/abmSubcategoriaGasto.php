...
if($operacion=="editar")
{
$consulta1="Update subcategoria_gasto set nombre=?, id_categoria_gastoFK=?, estado=? where id_subcategoria_gasto=?";
$stmt1 = $mysqli->prepare($consulta1);
$ss='ssss';
...
function buscaroption($id_categoria)
{
	...
	if($id_categoria==""){
		$informacion =array("1" => "exito","2" => "<option  value='' >SELECCIONAR</option>","3" => "0","4"=>"");
		echo json_encode($informacion);
		exit;
	}

		$sql= "Select nombre,id_subcategoria_gasto from subcategoria_gasto where estado='Activo' and id_categoria_gastoFK = ? order by nombre asc  ";
   ...
   $stmt = $mysqli->prepare($sql);
if (!$stmt) {
	$informacion =array("1" => "error","2" => $mysqli->error);
	echo json_encode($informacion);
	exit;
}
$ss='s';
$stmt->bind_param($ss,$id_categoria);

if ( ! $stmt->execute()) {
   $informacion =array("1" => "error","2" => $stmt->error);
   echo json_encode($informacion);
   exit;
}
...
{
	if($id_categoria==""){
		$informacion =array("1" => "exito","2" => "","3"=> "0");
		echo json_encode($informacion);
		exit;
	}
	...
	$mysqli=conectar_al_servidor();
	 $pagina='';
        ...
        from subcategoria_gasto where estado='Activo' and id_categoria_gastoFK = ? and nombre = ? order by nombre asc ";
		}else{
		$sql= "Select id_subcategoria_gasto,nombre,estado,id_categoria_gastoFK
        from subcategoria_gasto where estado='Activo' and id_categoria_gastoFK = ? order by nombre asc ";
		}
...
if($buscar != ""){
	$ss='ss';
	$stmt->bind_param($ss,$id_categoria,$buscar);
}else{
	$ss='s';
	$stmt->bind_param($ss,$id_categoria);
}
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  $pagina.="
			  <table class='$styleName' border='1' cellspacing='1' cellpadding='5'>
			  <tr id='tbSelecRegistro' onclick='ObtenerdatosAbmSubcategoriaGasto(this)'>
